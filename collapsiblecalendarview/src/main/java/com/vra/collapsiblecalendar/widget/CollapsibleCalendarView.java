package com.vra.collapsiblecalendar.widget;
...
import com.vra.collapsiblecalendar.R;
import com.vra.collapsiblecalendar.view.BounceAnimator;
import com.vra.collapsiblecalendar.view.CalendarAdapter;
import com.vra.collapsiblecalendar.view.Day;
import com.vra.collapsiblecalendar.view.Event;
import com.vra.collapsiblecalendar.view.ExpandIconView;
import com.vra.collapsiblecalendar.view.UICalendar;
...
public class CollapsibleCalendarView extends UICalendar implements View.OnClickListener {
    ...
    private void init(Context context) {
        ...
        mBtnPrevMonth.setOnClickListener(mBtnPrevMonth -> prevMonth());

        mBtnNextMonth.setOnClickListener(mBtnNextMonth -> nextMonth());

        mBtnPrevWeek.setOnClickListener(mBtnPrevWeek -> prevWeek());

        mBtnNextWeek.setOnClickListener(mBtnNextWeek -> nextWeek());

        mTodayIcon.setOnClickListener(mTodayIcon -> changeToToday());
    ...
    public void reload() {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            if (params != null && (mAdapter.getItem(i).getDiff() < params.prevDays || mAdapter.getItem(i).getDiff() > params.nextDaysBlocked)) {
                ...
                final int pos = i;
                view.setOnClickListener(view1 -> onItemClicked(view, mAdapter.getItem(pos)));
            }
            rowCurrent.addView(view);
        }
    ...
    public void setAdapter(CalendarAdapter adapter) {
        mAdapter = adapter == null ? new CalendarAdapter(getContext()) : adapter;
    ...
    private void collapseTo(int index) {
        if (state == STATE_COLLAPSED) {
            ...
            this.post(() -> mScrollViewBody.smoothScrollTo(0, topHeight));
